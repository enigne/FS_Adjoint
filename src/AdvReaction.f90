...
!> \ingroup Solvers
!-----------------------------------------------------------------------------
SUBROUTINE AdvReactionSolver( Model,Solver,dt,TransientSimulation )
  USE DefUtils
  USE ElementDescription
  USE Differentials
  USE MaterialModels
...
! #endif
  ...
  REAL(KIND=dp), ALLOCATABLE :: ReactionCoeff(:) 
  ...
  SAVE STIFF, MASS, SourceFunc, FORCE, &
       ...
       ResidualVector, StiffVector, MeshVelocity, ReactionCoeff
  !------------------------------------------------------------------------------
  ...
  SolverName = 'AdvectionReactionSolver ('// TRIM(Solver % Variable % Name) // ')'
...
!============================== Get the reaction term =================================
          ReactionCoeff(1:n) = GetReal( BodyForce, TRIM(VariableName) // ' Reaction Coefficient',Found )
          IF (.NOT.Found) ReactionCoeff(1:n) = 0.0_dp
!======================================================================================
        END IF
        ...
        CALL LocalMatrix( STIFF, MASS, FORCE,&
             ...
             Flux, NormalFlux, SubstantialSurface, ALEFormulation, ReactionCoeff)
   ...
   CONTAINS
     ...
     SUBROUTINE LocalMatrix( STIFF, MASS, FORCE,&
       ...
       REAL(KIND=dp) ::&
            ...
            Velo(:,:), MeshVelo(:,:), OldFreeSurf(:), Flux(:,:), ReactionCoeff(:)
       ...
       DO t = 1,IntegStuff % n
          ...
          ReacGauss = SUM( Basis(1:nCoord) * ReactionCoeff(1:nCoord) )
          DO i=1,DIM-1
          ...
          DO p=1,n
             DO q=1,n
                ...
                END DO
                STIFF(p,q) =  STIFF(p,q) + s * ReacGauss * Basis(q) * Basis(p)
     ...
     !------------------------------------------------------------------------------
   END SUBROUTINE AdvReactionSolver
!------------------------------------------------------------------------------
